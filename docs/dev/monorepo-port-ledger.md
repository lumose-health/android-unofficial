...
had an independent, equivalent fix; there is no monorepo commit to cherry-pick, so record both PR
links instead).
...
- Status legend: `pending` (not yet cherry-picked) -> `ported` (cherry-pick landed on this repo's
  `develop`); or, for a row with no commit to cherry-pick, `parallel-landed` (this repo already
  carried an independent, equivalent fix -- terminal on creation, record both PR links in the Port
  status column).
...
from `pending` to `ported` when the cherry-pick reaches this repository's `develop`. A row with
no commit to cherry-pick (this repo already had an independent fix) is recorded as
`parallel-landed` instead, and is terminal from the start.
